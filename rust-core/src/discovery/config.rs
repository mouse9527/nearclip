...
mod tests {
    ...
    fn test_discovery_config_validation() {
        ...
        let mut invalid_config = DiscoveryConfig::default();
        invalid_config.timeout_seconds = 0;
        assert!(invalid_config.validate().is_err());
    }

    #[test]
    fn test_discovery_config_convenience_methods() {
        // Test convenience factory methods
        let test_config = DiscoveryConfig::for_testing();
        assert_eq!(test_config.timeout_seconds, 5);
        assert_eq!(test_config.max_retry_count, 1);
        assert!(!test_config.enable_auto_restart);
        assert_eq!(test_config.discovery_name, "Test Discovery");
        
        let high_perf_config = DiscoveryConfig::for_high_performance();
        assert_eq!(high_perf_config.timeout_seconds, 15);
        assert_eq!(high_perf_config.max_retry_count, 5);
        assert!(high_perf_config.enable_auto_restart);
        assert_eq!(high_perf_config.discovery_name, "High Performance Discovery");
        
        let battery_config = DiscoveryConfig::for_battery_saving();
        assert_eq!(battery_config.timeout_seconds, 60);
        assert_eq!(battery_config.max_retry_count, 2);
        assert!(!battery_config.enable_auto_restart);
        assert!(!battery_config.enable_ble_discovery);
        assert_eq!(battery_config.discovery_name, "Battery Saving Discovery");
    }
}